...
## How do I control low VRAM mode?

Ollama reduces certain defaults (such as context length) when the combined VRAM across your GPUs is below 20 GiB. You can raise or lower this cutoff by setting `OLLAMA_LOW_VRAM_THRESHOLD` to the total VRAM (in bytes) that should trigger the warning. For example, to set the threshold to 12 GiB run:

```powershell
$env:OLLAMA_LOW_VRAM_THRESHOLD = 12884901888
ollama serve
```

Set the variable before starting `ollama serve` (or add it to your system environment) and the server will log whether it is running in low-VRAM mode using the updated value.
